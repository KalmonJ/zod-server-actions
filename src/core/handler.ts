...
export const createAction = <I extends ZodTypeAny, C extends object, R>(
  props: HandlerProps<I, C, R>,
) => {
  ...
  return async function <V extends I>(input: V) {
    ...
    try {
      ...
      if (props.config && props.config.debug) logger.debug("Request handler success: ", data);
